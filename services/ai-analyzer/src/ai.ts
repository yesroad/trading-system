...
export async function callAiFilter(params: {
  context: string;
}): Promise<{ result: AiFilterDecision; usage?: AiUsage; model?: string }> {
  const system = [
...
function safeParseDecision(text: string): AiFilterDecision {
  ...
  let obj: unknown;
  try {
    obj = JSON.parse(cleaned) as unknown;
  } catch {
  ...
  if (typeof decision !== 'string' || !['ALLOW', 'CAUTION', 'BLOCK'].includes(decision)) {
